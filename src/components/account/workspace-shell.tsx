...
const menuItems = [
  ...
  { label: "Book Orders", href: "/account/orders" },
] as const;
...
export function AccountWorkspaceShell({
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem("account_sidebar_collapsed", collapsed ? "1" : "0");
  ...
  const activeHref = useMemo(() => {
    ...
    if (pathname.startsWith("/account/donations")) return "/account/donations";
    return "/account";
  }, [pathname]);
    ...
    <section className="flex h-screen w-full bg-gradient-to-br from-slate-50 via-[#e7f2fb] to-[#d8eaf7] text-gray-900 overflow-hidden">
      {mobileNavOpen ? (
        <button
          ...
          className="fixed inset-0 z-40 bg-[#0c4669]/45 sm:hidden"
      ...
      >
        ...
        <div className={`h-16 flex items-center border-b border-white/10 ${collapsed ? "px-2 sm:justify-center" : "px-4"}`}>
          ...
          <span className={`font-bold text-lg ${collapsed ? "sm:hidden" : ""}`}>Workspace</span>
          <button
            type="button"
        ...
        <div className="relative z-10 p-4 border-t border-white/10">
          <button
            type="button"
            className="w-full inline-flex items-center justify-center rounded-lg bg-white/10 px-4 py-2.5 text-sm font-semibold text-white hover:bg-white/20"
        ...
        <header className="h-16 bg-white/95 backdrop-blur border-b border-[#d6e6f2] flex items-center justify-between px-4 sm:px-6">
          <div className="flex items-center gap-4">
            <button
              ...
              className="sm:hidden p-2 -ml-2 text-gray-500 hover:text-[#1a2849] rounded-lg hover:bg-gray-100"
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">{subtitle || email}</p>
            </div>
          </div>
